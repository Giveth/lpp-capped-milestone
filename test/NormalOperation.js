...
describe('LiquidPledging test', function() {
  ...
  it('Should deploy the plugin', async () => {
    await factory.deploy(liquidPledging.$address, 'Milestone1', 'URLMilestone1', 0, recipient, utils.toWei('1'), milestoneReviewer, campaignReviewer, accounts[0], accounts[1], { from: adminMilestone1, gas:5000000 }).on('receipt', console.log);
    const nAdmins = await liquidPledging.numberOfPledgeAdmins();
  ...
  it('Should make a donation to milestone', async () => {
    await liquidPledging.donate(1, 3, { from: donor1, value: '1000' }).on('receipt', console.log);
    const nPledges = await liquidPledging.numberOfPledges();
  ...
  it('Should mark milestone Completed', async () => {
    await milestone.acceptMilestone({ from: milestoneReviewer}).on('receipt', console.log);
    assert.equal(await milestone.accepted(), true);
  });

  it('Should withdraw pledge for completed milestone', async () => {
    await milestone.withdraw(3, '1000', { from: recipient}).on('receipt', console.log);
  ...
  it('Should confirm payment and complete withdraw', async () => {
    ...
    await vault.confirmPayment(0).on('receipt', console.log);
  ...
  it('Recipient should be able to collect from contract', async () => {
    ...
    let { gasUsed } = await milestone.collect({ from: recipient, gasPrice: 1 }).on('receipt', console.log);
